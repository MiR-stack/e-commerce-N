...
const ProductsList = () => {
  ...
  // const statusValues = ['instock', 'outOfStock', 'lowStock'];
  // const statusLen = statusValues.length;
  // const [statusIndex, setStatusIndex] = useState(0);
  // const [status, setStatus] = useState(null);

  // const handleNextStatus = () => {
  //   if (statusIndex >= statusLen) {
  //     setStatusIndex(0);
  //     setStatus(statusValues[0]);
  //   } else {
  //     setStatusIndex(statusIndex + 1);
  //     setStatus(statusValues[statusIndex + 1]);
  //   }
  // };
  ...
  // get products
  const { data } = useGetProductsQuery({ page, limit: pageLimit, search, status, ...sorting });
  ...
  return (
    <>
      ...
      <Row className="g-0 mb-2 d-none d-lg-flex">
        ...
        <Col>
          <Row className="g-0 h-100 align-content-center custom-sort ps-5 pe-4 h-100">
            <Col xs="3" className="d-flex flex-column mb-lg-0 pe-3 d-flex" onClick={() => handleSort('name')}>
              <div className="text-muted text-small cursor-pointer sort">TITLE</div>
            </Col>
            <Col xs="2" lg="3" className="d-flex flex-column pe-1 justify-content-center" onClick={() => handleSort('stock_quantity')}>
            ...
            <Col xs="2" lg="3" className="d-flex flex-column pe-1 justify-content-center">
              <div className="text-muted text-small cursor-pointer sort" onClick={() => handleSort('base_price')}>
                PRICE
              </div>
            </Col>
            <Col xs="2" lg="3" className="d-flex flex-column pe-1 justify-content-center">
      ...
      {data?.data.map((product) => {
        const { name, sale_price: price, id, stock_quantity: quantity, status: productStatus, images } = product;
        const image = images.find((item) => item.is_main);
        return (
          <Card className={`mb-2 ${selectedItems.includes(1) && 'selected'}`} key={id}>
            <Row className="g-0 h-100 sh-lg-9 position-relative">
              <Col xs="auto" className="positio-relative h-100" onClick={() => handleNavigation(id)}>
                <img src={getImageUrl(image?.image_data.url)} alt={image?.alt_text} className="card-img card-img-horizontal sw-11 h-100" />
              </Col>
